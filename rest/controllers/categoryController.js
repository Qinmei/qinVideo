...
class categoryController {
  ...
  static async category_post(ctx) {
    ...
    if (user.level > 99) {
      ...
      const data = await CategoryModel.create(post).catch(err => {
        return { code: 404, msg: err.message };
      });
      ctx.send({ data });
    } else {
  ...
  static async category_get(ctx) {
    const { id } = ctx.params;
    const data = await CategoryModel.findById(id).catch(err => {
      return { code: 404, msg: err.message };
    });
    ctx.send({ data });
  }
  ...
  static async category_put(ctx) {
    ...
    const { user } = ctx.state;
    if (user.level > 99) {
      const result = await CategoryModel.findByIdAndUpdate(id, data).catch(
        err => {
          return { code: 404, msg: err.message };
        }
      );
  ...
  static async category_delete(ctx) {
    ...
    if (user.level > 9) {
      ...
      const data = await CategoryModel.findByIdAndDelete(id).catch(err => {
        return { code: 404, msg: err.message };
      });
      ctx.send({ data });
    } else {
  ...
  static async category_delete_batch(ctx) {
    ...
    if (user.level > 99) {
      if (type === "all") {
        const data = await CategoryModel.remove({}).catch(err => {
          return { code: 404, msg: err.message };
        });
        ctx.send({ data });
      } else {
        const data = await CategoryModel.remove({ _id: { $in: list } }).catch(
          err => {
            return { code: 404, msg: err.message };
          }
        );
