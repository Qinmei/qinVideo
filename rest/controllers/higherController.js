...
class higherController {
  ...
  static async list(ctx) {
    ...
    const save = async (data) => {
      ...
      data.play.level = 0;
      let result = true;
      await AnimateModel.findOneAndUpdate({ slug }, { $set: data }, { upsert: true }).catch(err => result = false)
      return result;
    }
  ...
  static async update(ctx) {
    ...
    const updateData = await AnimateModel.find({ 'information.isUpdate': true });
  ...
  static async replace(ctx) {
    ...
    if (type === 'cover') {
      AnimateModel.find({ 'cover.vertical': { $regex: before, $options: 'i' } }).exec(function (err, docs) {
        docs.forEach(function (doc) {
          ...
          AnimateModel.updateOne({ _id: doc._id }, { $set: { "cover.vertical": vertical, "cover.horizontal": horizontal } }).catch(err => console.log(err))
        })
      })
    } else if (type === 'play') {
      AnimateModel.find().exec(function (err, docs) {
        docs.forEach(function (doc) {
          if (doc.eposide && doc.eposide[0]) {
            ...
            AnimateModel.updateOne({ _id: doc._id }, { $set: { eposide: newData } }).catch(err => console.log(err))
          }
        })
      })
    }
