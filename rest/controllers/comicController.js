...
class comicController {
    ...
    static async comic_put(ctx) {
        ...
        const data = await ComicModel.updateOne({slug},{$set:sendData}).catch(err=>{return {code:404,msg:err.message}});
        ctx.send({ data } );
    }
    ...
    static async comic_put_batch(ctx) {
        ...
        if(type === 'all'){
            const result = await ComicModel.update({}, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        }else{
            const result = await ComicModel.update({ slug: { $in: list } }, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        };
    }
    ...
    static async comic_play(ctx){
        ...
        const { user } = ctx.state;
        const result = await ComicModel.findOne({slug,level:{$lte:user.level}});
