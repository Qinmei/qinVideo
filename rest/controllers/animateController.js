...
const unwindList = ['$category.area','$category.year','$category.kind','$author'].map(item=>{
    return {$unwind: {
                "path":item,
...
const relativeLookup = ['like','unlike','play','comment','danmu'].map(item=>{
    if(item === 'like' || item=== 'unlike' ){
        ...
        return {$lookup:{
            ...
            let: { value:'$slug'},
            pipeline: [
                {$match:{$expr:{ $gt:[{$indexOfBytes: [ "$target", '$$value' ]},-1 ] }}},
            ],
            as: `relative.${item}`
        }}
    }else if(item === 'danmu'){
        return {$lookup:{
            ...
            let: { value:'$slug'},
            pipeline: [
                {$match:{$expr:{ $gt:[{$indexOfBytes: [ "$player", '$$value' ]},-1 ] }}},
            ],
            as: `relative.${item}`
        }}
    }
...
class animateController {
    ...
    static async animate_put(ctx) {
        ...
        const { user } = ctx.state;
        if(user.level < 100){
            const single = await AnimateModel.findOne({slug:slug,author:user._id});
            if(!single) return ctx.error({msg:'没有权限',code:402})
            sendData.stats && delete sendData.status;
            sendData.slug && delete sendData.slug;
        };
        const data = await AnimateModel.updateOne({slug},{$set:sendData}).catch(err=>{return {code:404,msg:err.message}});
        ctx.send({ data } );
    }
    ...
    static async animate_put_batch(ctx) {
        ...
        if(type === 'all'){
            const result = await AnimateModel.update({}, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        }else{
            const result = await AnimateModel.update({ slug: { $in: list } }, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        };
    }
    ...
    static async animate_play(ctx){
            ...
            ...['play','comment','danmu'].map(item=>{
                if(item === 'comment'){
                    ...
                    return {$lookup:{
                        from: 'datas',
                        pipeline: [
                            {$match:{$expr:{$eq: ['$belong', `${slug}S${season}E${eposide}`] }}},
                        ],
                        as: `relative.${item}`
                    }}
                }else if(item === 'danmu'){
        ...
        if(config){
            if(animate.play.kind === 'mp4' || animate.play.kind === 'm3u8'){
                const configPrefix = config.playLimit.filter(item=>item.level <= animate.play.level)
                ...
                if(configPrefix){
                    playLink = configPrefix.prefix + animate.play.linkPrefix + animate.playInfo.link;
                }
            }
        }
