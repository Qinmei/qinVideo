...
const unwindList = ['$author','$title','$title.animate','$title.post','$title.comic'].map(item=>{
    return {$unwind: {
                "path":item,
...
class commentController {
    ...
    static async comment_put(ctx) {
        ...
        const data = await CommentModel.updateOne({_id:id},{$set:sendData}).catch(err=>{return {code:404,msg:err.message}});
        ctx.send({ data } );
    }
    ...
    static async comment_put_batch(ctx) {
        ...
        if(type === 'all'){
            const result = await CommentModel.update(commentQuery, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        }else{
            const result = await CommentModel.update({ _id: { $in: list } }, {$set:data}, {multi: true}).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data:result } );
        };
    }
    ...
    static async comment_delete_batch(ctx) {
        ...
        if(type === 'all'){
            const data = await CommentModel.remove(commentQuery).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data } );
        }else{
            const data = await CommentModel.remove({ _id: { $in: list } }).catch(err=>{return {code:404,msg:err.message}});
            ctx.send({ data } );
        };
    }
