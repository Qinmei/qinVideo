...
class ReportController extends Controller {
	async query() {
		...
		const result = await service.report.query(query).catch(() => 23000);
	...
	async info() {
		...
		const result = await service.report.info(id).catch(() => 23001);
		ctx.helper.send(result);
	}
	...
	async destroyMany() {
		...
		const result = await service.report.destroy(ids).catch(() => 23004);
		ctx.helper.send(result);
	}
}
