...
class AuthController extends Controller {
  ...
  async noExistByName(oldName: string) {
    ...
    if (name.replace(/[\u4e00-\u9fa5]/g, "aa").length < 6) {
      ctx.helper.error(10020);
    }

    if (name.replace(/[\u4e00-\u9fa5]/g, "aa").length > 30) {
      ctx.helper.error(10021);
    }
  ...
  async generateToken(userInfo: any) {
    const { ctx } = this;
    const { _id, email, name, level, score, avatar, background, introduce } =
      userInfo;
  ...
  async resetPasswordMail() {
    ...
    service.utils.sendMail({
      ...
      text: "正文如下:",
      html:
        `<h3>亲爱的${name}:<h3><p>您正在进行重置密码的操作,如果不是您本人所为请忽略此邮件,确认重置密码请复制点击下方验证码, 有效期两小时:</p><p style='margin-left:30px;font-size:20px'>${token}</p>`,
    });
  ...
  async resetPasswordAuth() {
    ...
    try {
      ...
      result = await service.user.update(
        [id],
        { password: newPass, refreshToken },
      ).catch(() => 10011);
    } catch (err) {}
  ...
  async sendVerifyCode() {
    ...
    service.utils.sendMail({
      ...
      text: "正文如下:",
      html:
        `<h3>亲爱的${name}:<h3><p>请点击下方链接进行账户验证, 有效期两小时:</p><p style='margin-left:30px;font-size:20px'>${link}</p>`,
    });
